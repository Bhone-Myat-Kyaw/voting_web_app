...
const OnBoardingModal = ({isOpen, setShowOnboardingModal}: Props) => {
  ...
  return (
    <div className='fixed inset-0 bg-transparent backdrop-blur-sm flex items-center justify-center z-100' >
        <div className='bg-cwhite rounded-3xl shadow-light py-7 px-9 w-[30%] min-w-80 '>
            <div className='flex flex-col gap-5 mb-5'>
                
                <h2 className='text-h2 font-heading-bold text-center'>Select your role to begin.</h2>

                <motion.button onClick={()=>redirectHome(People.Freshmen)} className='px-4 py-2 bg-blue-300 text-blue-700 text-button rounded-3xl w-full font-button-bold hover:text-cwhite hover:bg-primary capitalize'>{People.Freshmen}</motion.button>
